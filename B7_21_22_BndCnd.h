...
void B7_21_22_BndCnd(double CRO1[MY][MX], double CU1[MY][MX],
                     double CV1[MY][MX], double CE1[MY][MX],
                     double CRO2[MY][MX], double CU2[MY][MX],
                     double CV2[MY][MX], double CE2[MY][MX]) {
  ...
  for (Y = 1; Y <= Nyp; Y++) { // DO K = 1,Nyp
    ...
    if (BoundCondRight == 1) { // ÂÍÈÌÀÍÈÅ - ÐÀÍÜØÅ ÁÛË Npr
      ...
      CRO1[Y][Nxp] = CRO1[Y][Nxp - 4];
      CRO1[Y][Nxp - 1] = CRO1[Y][Nxp - 3];
      CU1[Y][Nxp] = -CU1[Y][Nxp - 4];
      CU1[Y][Nxp - 1] = -CU1[Y][Nxp - 3];
      CV1[Y][Nxp] = CV1[Y][Nxp - 4];
      CV1[Y][Nxp - 1] = CV1[Y][Nxp - 3];
      CE1[Y][Nxp] = CE1[Y][Nxp - 4];
      CE1[Y][Nxp - 1] = CE1[Y][Nxp - 3];
      CRO2[Y][Nxp] = CRO2[Y][Nxp - 4];
      CRO2[Y][Nxp - 1] = CRO2[Y][Nxp - 3];
      CU2[Y][Nxp] = -CU2[Y][Nxp - 4];
      CU2[Y][Nxp - 1] = -CU2[Y][Nxp - 3];
      CV2[Y][Nxp] = CV2[Y][Nxp - 4];
      CV2[Y][Nxp - 1] = CV2[Y][Nxp - 3];
      CE2[Y][Nxp] = CE2[Y][Nxp - 4];
      CE2[Y][Nxp - 1] = CE2[Y][Nxp - 3];
    } // end if
  } // ENDDO
