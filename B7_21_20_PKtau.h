...
void B7_21_20_PKtau(double Tau, double OldRO1[MY][MX], double OldU1[MY][MX],
  ...
  for (Y = 3; Y <= Nyp - 2; Y++) {   // DO K = 3,Nyp-2
    for (X = 3; X <= Nxp - 2; X++) { // DO I = 3,Nxp-2
      ...
      if (p1N / (p1N + p2N / om) < concmin) {
        ...
        a_E = (RO2[Y][X] * (E2[Y][X] + vu / 2) - Tau * LE2[Y][X]) / p2N -
              vun / 2.0;
        ...
        vun = pow(a_U, 2) + pow(a_V, 2);
        a_E = (RO1[Y][X] * (E1[Y][X] + vu / 2) - Tau * LE1[Y][X]) / p1N -
              vun / 2.0;
  ...
  B7_21_22_BndCnd(NewRO1, NewU1, NewV1, NewE1, NewRO2, NewU2, NewV2,
                  NewE2); // Первый (из двух) шаг Рунге-Кутта.
