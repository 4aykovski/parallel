...
double vis1(double t1, double t2, double ro1, double ro2) {
  ...
  d1 = D211_Return_Diffu(t1, t2, ro1, ro2); // ÁÛËÎ r01, גלוסעמ r01!!!!!!!!!
...
double vis2(double t1, double t2, double ro1, double ro2) {
  ...
  x2 = ro2 / (ro1 * om + ro2);
  d1 = D211_Return_Diffu(t1, t2, ro1, ro2);
...
double dHdH(double vis2xy, double vis1xy, double uve2, double uve1, double uve0,
            double h2, double h1) {
...
double dXdY(double vis2x, double vis0x, double uv22, double uv02, double uv20,
            double uv00, double h2x, double h1x, double h2y, double h1y) {
  return (vis2x * (uv22 - uv02) / (h2y + h1y) -
          vis0x * (uv20 - uv00) / (h2y + h1y)) /
         (h2x + h1x);

} // end function dxdy

double dYdX(double vis2y, double vis0y, double uv22, double uv20, double uv02,
            double uv00, double h2x, double h1x, double h2y, double h1y) {
  return (vis2y * (uv22 - uv20) / (h2x + h1x) -
          vis0y * (uv02 - uv00) / (h2x + h1x)) /
         (h2y + h1y);
} // end function dydx
...
double vis1t(double t1, double t2, double ro1, double ro2) {
        //!w = 1.0 / (dl * an0 * sig1 * *2 * 1.e5)
        //!vis1 = w * 5.0 / 32.0 * sqrt(t1 / 3.14) / (1 + ro2 / ro1 / om *
sqrt(2.0) / 4.0
        //!**sqrt(1 + t2 / t1 / om) * (1 + sig2 / sig1) * *2)
        vis = vis10 * t1 * *anu1;
        x2 = ro2 / (ro1 * om + ro2);
...
//!w = 1.0 / (dl * an0 * sig1 * *2 * 1.e5)
//!vis2 = w * 5.0 / 32.0 * sqrt(t2 / 3.14 * om) * (sig1 / sig2) * *2 /
//!*(1 + 5.0 / 12.0 * ro1 / ro2 * sqrt(1 + t1 / t2 * om) * (1 + sig1 / sig2) *
*2) vis = vis20 * t2 * *anu2; x2 = ro2 / (ro1 * om + ro2);
//!d1 = diffu(t1, t2, ro1, ro2, anu12, d0)
