...
void B3_BndCnd()
//	double CRO1[MY][MX],double CU1[MY][MX],double CV1[MY][MX],double
//CE1[MY][MX], 	double CRO2[MY][MX],double CU2[MY][MX],double CV2[MY][MX],double
//CE2[MY][MX])
{
  ...
  for (Y = 1; Y <= Nyp; Y++) { // DO K = 1,Nyp
    ...
    if (BoundCondRight == 1) { // ÂÍÈÌÀÍÈÅ - ÐÀÍÜØÅ ÁÛË Npr
      ...
      GRO1[Y][Nxp] = GRO1[Y][Nxp - 4];
      GRO1[Y][Nxp - 1] = GRO1[Y][Nxp - 3];
      GU1[Y][Nxp] = -GU1[Y][Nxp - 4];
      GU1[Y][Nxp - 1] = -GU1[Y][Nxp - 3];
      GV1[Y][Nxp] = GV1[Y][Nxp - 4];
      GV1[Y][Nxp - 1] = GV1[Y][Nxp - 3];
      GE1[Y][Nxp] = GE1[Y][Nxp - 4];
      GE1[Y][Nxp - 1] = GE1[Y][Nxp - 3];
      GRO2[Y][Nxp] = GRO2[Y][Nxp - 4];
      GRO2[Y][Nxp - 1] = GRO2[Y][Nxp - 3];
      GU2[Y][Nxp] = -GU2[Y][Nxp - 4];
      GU2[Y][Nxp - 1] = -GU2[Y][Nxp - 3];
      GV2[Y][Nxp] = GV2[Y][Nxp - 4];
      GV2[Y][Nxp - 1] = GV2[Y][Nxp - 3];
      GE2[Y][Nxp] = GE2[Y][Nxp - 4];
      GE2[Y][Nxp - 1] = GE2[Y][Nxp - 3];
    } // end if
  } // ENDDO
