...
#include "D210_Return_AK.h"

// void M7_4_0_Modeling(double Tau, double om, double c1v, double c2v) {
void D20_PKtau(double Tau, double OldRO1[MY][MX], double OldU1[MY][MX],
               double OldV1[MY][MX], double OldE1[MY][MX],
               double OldRO2[MY][MX], double OldU2[MY][MX],
               double OldV2[MY][MX], double OldE2[MY][MX],
               double NewRO1[MY][MX], double NewU1[MY][MX],
               double NewV1[MY][MX], double NewE1[MY][MX],
               double NewRO2[MY][MX], double NewU2[MY][MX],
               double NewV2[MY][MX], double NewE2[MY][MX]) {

  // M7_200_Lio2D(1,K1RO1,K1RO2,K1U1,K1U2,K1V1,K1V2,K1E1,K1E2); //Расчёт нового
  // слоя "Первого" газа.
  // M7_200_Lio2D(2,K1RO1,K1RO2,K1U1,K1U2,K1V1,K1V2,K1E1,K1E2); //Расчёт нового
  // слоя "Второго" газа.
  ...
  for (Y = 3; Y <= Nyp - 2; Y++) {   // DO K = 3,Nyp-2
    for (X = 3; X <= Nxp - 2; X++) { // DO I = 3,Nxp-2
      ...
      ak1 = D210_Return_AK(OldRO1[Y][X], OldRO2[Y][X], OldU1[Y][X], OldU2[Y][X],
                           OldE1[Y][X] / c1v, OldE2[Y][X] / c2v);

      beta =
          OldE1[Y][X] / c1v / (OldE1[Y][X] / c1v + 1 / om * OldE2[Y][X] / c2v);
      ...
      if (p1N / (p1N + p2N / om) < concmin) {
        ...
        a_E = (RO2[Y][X] * (E2[Y][X] + vu / 2) - Tau * KE2[Y][X]) / p2N -
              vun / 2.0;
        ...
        vun = pow(a_U, 2) + pow(a_V, 2);
        a_E = (RO1[Y][X] * (E1[Y][X] + vu / 2) - Tau * KE1[Y][X]) / p1N -
              vun / 2.0;
        ...
        q1 = -KE1[Y][X] + ak1 * c1UN * (c2UN - c1UN) +
             ak1 * beta * pow(c1UN - c2UN, 2) + ak1 * c1VN * (c2VN - c1VN) +
             ak1 * beta * pow(c1VN - c2VN, 2);

        q2 = -KE2[Y][X] + ak1 * c2UN * (c1UN - c2UN) +
             ak1 * (1 - beta) * pow(c1UN - c2UN, 2) +
             ak1 * c2VN * (c1VN - c2VN) +
             ak1 * (1 - beta) * pow(c1VN - c2VN, 2);
        ...
        b1 =
            Tau * q1 +
            RO1[Y][X] * (E1[Y][X] + (pow(U1[Y][X], 2) + pow(V1[Y][X], 2)) / 2) -
            p1N * (pow(c1UN, 2) + pow(c1VN, 2)) / 2;

        b2 =
            Tau * q2 +
            RO2[Y][X] * (E2[Y][X] + (pow(U2[Y][X], 2) + pow(V2[Y][X], 2)) / 2) -
  ...
  // B7_21_22_BndCnd(NewRO1,NewU1,NewV1,NewE1,  NewRO2,NewU2,NewV2,NewE2);
  // //Первый (из двух) шаг Рунге-Кутта.
