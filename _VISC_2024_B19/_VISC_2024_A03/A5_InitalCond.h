  ...
  for (y = 3; y <= Nyp; y++) {   // do k = 3,nyp
    for (x = 1; x <= Nxp; x++) { // do i = 1,nxp
      ...
      if ((XVV[x] < x3 - 0.01 * h0) && (abs(cmax - 1) > 0.0001)) {
        gam =
            (al1p * gam1 * c1v + al2p * gam2 * c2v) / (al1p * c1v + al2p * c2v);
        pp = p1p + p2p;
        ...
        E2[y][x] = c2v * tem;
        U2[y][x] =
            sqrt(2 * (al1p * r1 + al2p * r2) / ((gam + 1) * d2d1 + gam - 1)) *
            (d2d1 - 1);
        U1[y][x] = U2[y][x];
        //!				vv0=U1[k][i]
      } // end if
...
/*
//subroutine raspad(pp, pm, up, um, rop, rom, gam1, gam2, ucd, al1m, c1v, al2m,
c2v) void raspad() {
...
int i;
        ...
        pcd = (pp + pm) / 2;
        ucd = (up + um) / 2;
        gam = (al1m * gam1 * c1v + al2m * gam2 * c2v) / (al1m * c1v + al2m *
c2v);
//222	continue
Label222: //continue
//	if (Pcd.GT.Pm) then
        if (Pcd > Pm) {
                //A = SQRT(((Gam1 + 1.) / 2. * Pcd + (Gam1 - 1) / 2. * Pm) *
ROm) A = SQRT(((Gam1 + 1.) / 2. * Pcd + (Gam1 - 1) / 2. * Pm) * ROm);
        }
        else {
                //A = (Gam1 - 1) / 2. / Gam1 * SQRT(Gam1 * Pm * ROm)
                A = (Gam1 - 1) / 2. / Gam1 * SQRT(Gam1 * Pm * ROm);
                //A = A*(1.-Pcd/Pm) / (1.-(Pcd/Pm)**((Gam1-1)/2./Gam1))
                A = A * (1. - Pcd / Pm) / (1. - (Pcd / Pm) * *((Gam1 - 1) / 2. /
Gam1));
        //end if
        }
