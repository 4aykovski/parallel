...
void A3_0_Grid() {
  ...
  cout << fixed << "X:   " << Xmin << "\t" << xN * 100 << "\t" << xK * 100
       << "\t" << Xmax << endl;
  cout << fixed << "Y:   " << "-" << intYmin / 100 << intYmin / 10
       << abs(Ymin - intYmin) << "\t" << yN * 100 << "\t" << yK * 100 << "\t"
       << Ymax << "\n\n";
...
void A3_1_GridX() {
  ...
  Nx = int((xxx + 0.1 * h0) / h0) + 1; //! Кол - во Х - полос в основной
                                       //! области.
  ...
  // Заполн.вспомогат.векторов для сетки Влево. От X=0; До Гр.Осн.Обл.(до
  // Первого доп.слоя).
  while (XYVVG[Ng] > xN) {
    Ng++;
    HGXY[Ng] = hx0;
  ...
  while (XYVVG[Ng] > -Gxy3) {
    if (XYVVG[Ng] > xN - Gxy1) { // На Первом дополнительном слое.
      ...
      qm = qm2;
    } else { //(XYVVG[Ng] > -Gxy3) На Третьем дополнительном слое. До самого
             //края.
      qm = qm3;
    }
    Ng++;
  ...
  /// Копирование с инверсией(разворотом) вспомогательных векторов "Рабочую"
  /// сетку.
  for (x = 1; x <= Ng; x++) {
    HX[x] = HGXY[Ng - x + 1];
    XVV[x] = XYVVG[Ng - x + 1];
  ...
  // Заполнение "Рабочей" сетки Вправо от Х=0; До  Гр.Осн.Обл.(до Первого
  // доп.слоя).
  while (XVV[x] < xK) {
    x++;
    HX[x] = hx0;
  ...
  while (XVV[x] < GRight3) {
    if (XVV[x] < xK + Gxy1) { // На Первом дополнительном слое.
      ...
      qm = qm2;
    } else { //(XVV[i] < Gxy3). //На Третьем дополнительном слое. До самого
             //края.
      qm = qm3;
    }
    x++;
...
void A3_2_GridY() {
  ...
  Ny = (yK + 0.1 * h0) /
       h0; //! Количество Y - полос в верхней части осн.области.
  ...
  if (BoundCondLower == 1) { // Nd
    ...
    // Заполн.вспомогат.векторов для сетки Вниз. От Y=0; До Гр.Осн.Обл.(до
    // Первого доп.слоя).
    while (XYVVG[Ng] > yN) {
      Ng++;
      HGXY[Ng] = hy0;
    ...
    while (XYVVG[Ng] > -Gxy3) {
      ...
      if (XYVVG[Ng] > yN - Gxy1) { // На Первом дополнительном слое.
        qm = qm1;
                 ...
                 yN - Gxy1 - Gxy2) { // На Втором дополнительном слое.
        qm = qm2;
      } else { //(XYVVG[Ng] > -Gxy3). На Третьем дополнительном слое. До самого
               //края.
        qm = qm3;
  ...
  // Заполнение "Рабочей" сетки Вверх от Y=0; До  Гр.Осн.Обл.(до Первого
  // доп.слоя).
  while (YVV[y] < yK) {
    y++;
    HY[y] = hy0;
